...
/* ========================= */
/* EDIT THESE WHEN DEBUGGING */

const testFilePattern = 'test-*.js'
let browsers = []
// browsers.push('phantomjs') // broken -- shuts down before output
browsers.push('firefox')
browsers.push('chromium')
// browsers = null // uses in all browsers on system
...
const filenames = glob.sync(testFilePattern, {cwd: 'test'})
fs.writeFileSync('test/run-this-in-browser.js',
                 '// auto-generated by browser-test.js\n' +
                 `window.localStorage.debug = '${debugging}'\n` +
                 setupLine +
                 filenames.map(x => `require('./${x}')\n`).join(''))

run('test/run-this-in-browser', {newServer, browsers})
